...
Ya = history_df.loc[:, ['loss']]
Yb = history_df.loc[:, ['val_loss']]
plt.ylabel("Loss")
plt.xlabel("Epochs")



plt.plot(X, Ya, label = 'loss')
plt.plot(X, Yb, label = 'val_loss')
plt.legend()
plt.show()
...
# plt.title('Passing Yards/Game by Occurrences')
# sns.violinplot(ax = ax, data = teamGameStats.iloc[:, 4:5])
# plt.savefig(r'C:\Users\sox_b\PycharmProjects\NFL_Predictions\myFootballSite\predictor\templates\predictor\violinplot.jpg') 

# plt.show()
# # plt.scatter(x, y)
# # plt.show()
PassingYards = teamGameStats.iloc[:, 4:5]
RushinYards = teamGameStats.iloc[:, 5:6]

# concatenate both dataframe
df = pd.concat([PassingYards.assign(orig='PassingYards'), RushinYards.assign(orig='RushingYards')], axis=0)

# plot
plt.title('Passing Yards/Game and RushingYards by Occurrences')
ax = sns.violinplot( data=df, split=True)
